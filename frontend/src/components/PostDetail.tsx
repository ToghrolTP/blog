...
export function PostDetail() {
  ...
  const excerpt =
    currentTranslation.content.replace(/[#*_\[\]\n`]/g, " ").slice(0, 160)
      .trim() + "...";
            ...
            "image": post.thumbnailUrl
              ? [
                post.thumbnailUrl.startsWith("/")
                  ? `${window.location.origin}${post.thumbnailUrl}`
                  : post.thumbnailUrl,
              ]
              : [],
          ...
          <div className="flex items-center gap-2 min-w-0 max-w-full">
            ...
            <div className="flex gap-2 overflow-x-auto min-w-0 flex-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]" dir="ltr">
              {(() => {
                ...
                const englishTags = post.tags.filter((tText) =>
                  !isPersian(tText)
                ).sort();
