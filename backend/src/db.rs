...
pub async fn init_db(pool: &SqlitePool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _ = sqlx::query(
        "ALTER TABLE post_translations ADD COLUMN is_machine_translated BOOLEAN NOT NULL DEFAULT 0",
    )
    .execute(pool)
    .await;
    ...
    let legacy_rows = sqlx::query_as::<_, (String, String, String, String, i64)>(
        "SELECT id, title, summary, content, read_time FROM posts WHERE title IS NOT NULL",
    )
    .fetch_all(pool)
    .await;
    ...
    if cat_count.0 == 0 {
        let default_categories = vec![
            (
                ...
                "linux-logo",
                "Linux kernel, shell, and OS internals",
            ),
            (
                "cybersecurity",
                "Cybersecurity",
                "SYSTEMS & INFRASTRUCTURE",
                "shield-check",
                "Security analysis, cryptography, and audits",
            ),
            (
                "terminal",
                "Terminal",
            ...
            (
                ...
                "cpu",
                "Database design, microservices, and APIs",
            ),
            (
                "devops",
                "DevOps",
                "SOFTWARE DEVELOPMENT",
                "gear",
                "CI/CD pipelines, Docker, and automation",
            ),
            (
                "academic",
                "Academic",
                "SOFTWARE DEVELOPMENT",
                "book-open",
                "Computer science papers and research topics",
            ),
            (
                "ai",
                "AI",
